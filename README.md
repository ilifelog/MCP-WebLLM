...
| **双协议支持** | 原生实现 SSE 和 Streamable HTTP 两种 MCP 传输协议 |
...
- **`background.js`**（Service Worker）：负责维护到各 MCP 服务器的长连接，实现 `SSEParser`、`MCPConnection`、`ServerManager` 三层架构，处理 JSON-RPC 请求/响应的转发。
...
```bash
# 示例：使用 mcp-server-filesystem（Node.js）
npx @modelcontextprotocol/server-filesystem /你的/工作目录
...
现在你可以直接向 AI 提问，AI 会按照特定格式输出工具调用指令，扩展会自动检测并在侧边栏的 **调用** 标签页中创建执行卡片。
...
| 操作 | 说明 |
|------|------|
| **添加服务器** | 填写名称、URL、传输协议后点击 Add Server |
...
扩展支持同时连接任意数量的 MCP 服务器，所有服务器的工具会被聚合到统一的工具列表中，并带有所属服务器的标注。

当 AI 发出工具调用时，扩展会根据工具名称自动找到提供该工具的服务器并路由请求，整个过程对 AI 完全透明。

**示例场景**：同时连接"本地文件系统服务器"和"GitHub API 服务器"，让 AI 能够读取本地代码文件并对比 GitHub 上的版本。
...
**适用场景**：大多数现有 MCP 服务器（如 `mcp-server-filesystem`、`mcp-server-github` 等）
